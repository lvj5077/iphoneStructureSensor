...
void saveAsPNG(const std::string& outFile, unsigned int width, unsigned int height, unsigned short* data, bool binary = true) {
  if (binary) {
...
void process(const std::string& depthFile, const std::string& outBasename,
  ...
  while (frame < numDepthFrames) {
    ...
    // save to PNG
    // const std::string filename = outBasename + "_" + std::to_string(frame) + ".png";
    const std::string filename = std::to_string(frame) + ".png";
    saveAsPNG(filename, depthWidth, depthHeight, depth);
...
int main(int argc, char* argv[]) {
  if (argc >= 3) {
    ...
    std::cerr << "Usage: depth2PNG path/to/file.depth PNG_seq_basename [numDepthFrames]" << std::endl;
  }
  return 0;
}
